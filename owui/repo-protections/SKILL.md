...
- **Forks of a third-party upstream keep upstream's license + default
  branch, and get no `CODEOWNERS`** — skip those three; apply the rest
  (merge mode, branch protection) as normal, since those govern review in
  our fork. CODEOWNERS is our governance written into someone else's
  tree, and a non-upstream file is friction on every merge from upstream.
  With no CODEOWNERS present, `require_code_owner_reviews` is vacuous and
  the 1-approval rule still holds. "Third-party" = the parent's owner is
  not us; a fork of one of our own repos is not exempt.
...
Repos that argocd-image-updater pushes to (detected via `image-updater/` directory at the repo root) use a GitHub **Ruleset** named `main-protection-with-iu-bypass` instead of classic Branch Protection. The Ruleset enforces PR-review + no-force-push + no-deletion for humans, but two actors are in `bypass_actors`: the IU GitHub App (so it can `git push origin main` directly) **and Repository admin (RepositoryRole id 5)** — the latter is load-bearing for a solo operator, who can't self-approve their own PRs and would otherwise never be able to merge into a CD repo.
