...
0. **Before reading or editing ANYTHING in a repo, run `git fetch && git branch --show-current && git status --short`.** Not just before committing — before the *first* file read that will inform an edit. A stale local checkout produces confidently-wrong edits: reading a file before pulling shows you what used to be missing, not what's actually missing now (2026-08-09, a batch of 8 "fix" PRs turned into 7 no-ops because 7 of the 8 repos already had the fix and stale reads didn't show it). If you're resuming work on a branch whose PR already merged, don't keep committing to it — `git log --oneline -1 HEAD` vs `-1 origin/main` after a fetch tells you immediately; a squash-merge means your branch's own commit hash is never an ancestor of what actually landed, so build the next change from a fresh branch off `origin/main`, not on top of the old one.
1. **Never commit to or push `main` directly.** Always create a descriptive branch (`feature/...` or `fix/...`) first. Check `git branch --show-current` before committing — if it says `main`, branch NOW. **After creating a branch with `git branch <name>`, you are still on the old branch** — that command doesn't switch. Always follow it with `git checkout <name>` and confirm via `git branch --show-current` before your next commit or `gh pr create`; `gh pr create` silently uses whatever branch you're actually standing on, not the one you meant to.
